...
class Supplier(db.Model):
    ...
    previous_incidents = db.Column(
        db.Integer,
        default=0
    )
    indigenous_claim = db.Column(
        db.Boolean,
        default=False
    )
    ...
    def to_dict(self):
        return {
            ...
            'quality_score': (
                ...
                if self.quality_score else None
            ),
            'delivery_score': (
                float(self.delivery_score)
                if self.delivery_score else None
            ),
            'compliance_score': (
                float(self.compliance_score)
                if self.compliance_score else None
            ),
            'financial_stability_score': (
                float(self.financial_stability_score)
                if self.financial_stability_score else None
            ),
...
class Component(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name='check_component_category'
        ),
        CheckConstraint(
            indigenous_status.in_([
                'indigenous',
                'imported',
                'unknown'
            ]),
            name='check_indigenous_status'
        ),
        CheckConstraint(
            verification_status.in_([
                'pending',
            ...
            name='check_component_verification'
        ),
        CheckConstraint(
            risk_level.in_([
                'low',
            ...
            name='check_component_risk'
        ),
        Index(
            'idx_component_supplier',
            'supplier_id'
        ),
        Index(
...
class Document(db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name='check_document_verification'
        ),
    )
...
class RiskAssessment(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name='check_assessment_risk_level'
        ),
        CheckConstraint(
            'supplier_id IS NOT NULL OR component_id IS NOT NULL',
...
class RiskFactor(db.Model):
    ...
    risk_assessment_id = db.Column(
        db.Integer,
        db.ForeignKey('risk_assessments.id'),
        nullable=False
    )
...
class VerificationRecord(db.Model):
    ...
    checked_fields = db.Column(db.JSON)
    missing_fields = db.Column(db.JSON)
    documents_reviewed = db.Column(db.JSON)
